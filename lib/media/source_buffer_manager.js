...
shaka.media.SourceBufferManager.prototype.append_ = function(data) {
  ...
  try {
    ...
    if (typeof(this.sourceBuffer_.appendBuffer) == 'function') {
      this.sourceBuffer_.appendBuffer(data);
    } else {
      var _sb = this.sourceBuffer_;
      var eustart = new CustomEvent('updatestart', { 'detail': _sb });
      var eupdate = new CustomEvent('update', { 'detail': _sb });
      _sb.dispatchEvent(eustart);
      _sb.dispatchEvent(eupdate);
      _sb.append(new Uint8Array(data));
      var euend = new CustomEvent('updateend', { 'detail': _sb });
      _sb.dispatchEvent(euend);
    }
  } catch (exception) {
